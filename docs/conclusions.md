...
Adam — a **1.56×–1.67×** iteration-efficiency advantage over L-BFGS.
...
- **Deep L-BFGS memory** is the single largest speed lever: iterations-to-target
...
convergence speed on this smooth problem.
...
- **Fallback combinators are strictly dominant safety nets.**
`Fallback([L50, Anderson])` matches the fastest pure-L50 stack (45 iters)
because the Anderson residual solve supplies finite, curvature-aware direction
the instant the L-BFGS history degenerates.
...
**geometric pitfall, not a tuning artifact**, and the fixed clip is the reliable
safeguard.
...
are genuinely orthogonal and independently swappable. The cautionary findings —
particularly the adaptive trust-region's geometric stall — are first-class results
that sharpen, rather than undermine, the algorithm's central claim. The best-of-
breed converging stacks (42–45 iterations vs. 70 for L-BFGS and 263 for Adam)
demonstrate that a coherent, modular combiner can outperform monolithic classical
methods while remaining fully `jit`/`vmap`/`grad`-compatible.
